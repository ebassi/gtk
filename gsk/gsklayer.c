                                 ...
                                 GskLayer *origin)
{
  ...
  /* If we got to the top level layer, we request a layout phase */
  if (iter == NULL)
    {
      iter = gsk_layer_get_top_level (self);
      if (GSK_LAYER_PRIV (iter)->frame_clock != NULL)
        {
          gdk_frame_clock_request_phase (GSK_LAYER_PRIV (iter)->frame_clock,
  ...
  if (iter == NULL)
    {
      iter = gsk_layer_get_top_level (self);
      if (GSK_LAYER_PRIV (iter)->frame_clock != NULL)
        gdk_frame_clock_request_phase (GSK_LAYER_PRIV (iter)->frame_clock, GDK_FRAME_CLOCK_PHASE_LAYOUT);
